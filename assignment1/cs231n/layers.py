...
def svm_loss(x, y):
    ...
    m = len(y)
    scores = x
    margins = np.maximum(0, scores - scores[np.arange(m),y].reshape(-1, 1) + 1)
    margins[np.arange(m),y] = 0
    loss =  np.sum(margins) / m
    

    margins[margins > 0] = 1 
    margins[np.arange(m), y] = - np.sum(margins, axis = 1) 
    dx = margins / m

    #loss = np.maximum(0, scores)
...
def softmax_loss(x, y):
    ...
    #- (score[np.arange(len(y)), y] + np.log(np.sum(np.exp(score), axis = 1)))
